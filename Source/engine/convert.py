...
def time(n): return f'{n*1001}/30000s'
...
def build(data,name,camera_names=None):
 ...
 r=E.Element('fcpxml',version='1.8');res=element(r,'resources');element(res,'format',id='f1',name='FFVideoFormat1080p2997',frameDuration='1001/30000s',width=1920,height=1080)
 ...
 for si,s in enumerate(sessions):
  ...
  m=element(res,'media',id=f'mc{si}',name=s['multicam_name']);mc=element(m,'multicam',format='f1',tcStart='0s',tcFormat='DF',duration=time(s['end']-s['start']))
  for cam,n in sorted(names.items()):
   ...
   for src in sorted((x for x in s['media'] if x['camera']==cam),key=lambda x:x['start']):
    c=element(angle,'clip',name=Path(src['path']).name,offset=time(src['start']-s['start']),start=time(src['start']),duration=time(src['frames']),format='f1',tcFormat='DF')
    element(c,'video',ref=src['asset_id'],offset=time(src['start']),start=time(src['start']),duration=time(src['frames']))
 lib=element(r,'library');event=element(lib,'event',name=name);project=element(event,'project',name=name+'_Edit');seq=element(project,'sequence',format='f1',tcStart=time(origin),duration=time(end-origin),tcFormat='DF');sp=element(seq,'spine')
 for c in cuts:
 ...
 E.indent(r)
 return E.tostring(r,encoding='utf-8',xml_declaration=True),dict(origin=origin,end=end,cuts=cuts,sessions=[{k:v for k,v in s.items() if k!='media'} for s in sessions],camera_names=names)
